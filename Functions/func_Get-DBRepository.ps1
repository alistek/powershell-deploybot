...
<#
 # Script FileName: func_Get-DBRepository.ps1
 # Current Version: A02
 # Description: Retrieve DeployBot Repository or Repositories
    ...
    Process {
        ...
        If (-not $whatif) {
            ...
            If ($response) {           
                If ($name) {
                    If ($ID) {
                        $response | Where title -Match $name
                    } Else {
                        $response | Select -ExpandProperty entries | Where title -Match $name
                    } # Terminate If - ID
                } Else {
                    If ($ID) {
                        $response
